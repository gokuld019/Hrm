...
const DonutChart = ({ present = 0, halfDay = 0, late = 0, absent = 0, total = 0 }) => {
  const segments = [
    { color: "#22c55e", pct: present,  label: "Present"  },
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <svg width={180} height={180} viewBox="0 0 180 180">
        ...
        <text x={cx} y={cy - 4}  textAnchor="middle" fontSize={26} fontWeight="800" fill="#1e293b">
          {total}
        </text>
        <text x={cx} y={cy + 16} textAnchor="middle" fontSize={10} fontWeight="600" fill="#94a3b8" letterSpacing="0.5">
          TOTAL EMPLOYEES
        </text>
      </svg>

      {/* Legend — pill style with live counts */}
      <div className="grid grid-cols-2 gap-2 w-full">
        {segments.map(s => (
          <div key={s.label} className="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg bg-gray-50 border border-gray-100">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: s.color }} />
              <span className="text-[11px] text-gray-500 font-medium truncate">{s.label}</span>
            </div>
            <span className="text-[11px] font-bold text-gray-700 tabular-nums">{s.pct}</span>
          </div>
        ))}
      </div>
    </div>
...
const AttendanceStatCard = ({ present = 0, late = 0, absent = 0, total = 0, onClick }) => (
  <div
    onClick={onClick}
    className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:border-orange-200 transition-all cursor-pointer group"
  >
    <div className="flex items-start justify-between mb-3">
      <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white shrink-0 bg-orange-500 shadow-sm shadow-orange-200">
    ...
    <p className="text-xs text-gray-400 font-medium mb-1">Attendance Overview</p>
    ...
    <div className="flex items-center gap-1.5 mb-3 flex-wrap">
      <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full ring-1 ring-emerald-100">
        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
        {present} Present
      </span>
      {late > 0 && (
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full ring-1 ring-amber-100">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
          {late} Late
        </span>
      )}
      {absent > 0 && (
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full ring-1 ring-rose-100">
    ...
    <div className="flex items-center gap-1 text-xs font-semibold text-orange-500 group-hover:underline">
...
// ─── DASHBOARD LEAVE APPROVAL PANEL ──────────────────────────────────────────
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                {statsLoading
                  ...
                  : <>You have <span className="text-orange-500 font-semibold">{task.total ?? 0}</span> Total Tasks
                      {late > 0 && <> &amp; <span className="text-amber-500 font-semibold">{late}</span> Late</>}
                      {" "}&amp; <span className="text-rose-500 font-semibold">{absent}</span> Absent Today</>
                }
              </p>
            </div>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                ...
                <DonutChart present={present} halfDay={halfDay} late={late} absent={absent} total={totalEmp} />
              </div>
